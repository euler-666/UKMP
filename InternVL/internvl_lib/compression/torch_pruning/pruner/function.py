...
class BasePruningFunc(ABC):
    ...
    def _prune_parameter_and_grad(self, weight, keep_idxs, pruning_dim):
        if hasattr(weight, 'global_name'):
            name = weight.global_name
        pruned_weight = torch.nn.Parameter(torch.index_select(weight, pruning_dim, torch.LongTensor(keep_idxs).to(weight.device).contiguous()), requires_grad=weight.requires_grad)
        if hasattr(weight, 'global_name'):
            pruned_weight.global_name = name
        if weight.grad is not None:
            pruned_weight.grad = torch.index_select(weight.grad, pruning_dim, torch.LongTensor(keep_idxs).to(weight.device))
        if hasattr(weight, "acc_grad") and weight.acc_grad is not None:
            pruned_weight.acc_grad = torch.index_select(weight.acc_grad, pruning_dim, torch.LongTensor(keep_idxs).to(weight.device))
        device = weight.device
        return pruned_weight.to(device)
...
class EmbeddingPruner(BasePruningFunc):
    ...
    def prune_out_channels(self, layer: nn.Embedding, idxs: list) -> nn.Module:
        ...
        if "relative_attention_bias" in layer.weight.global_name:
            head_idxs = []
            for i in idxs:
                head_idxs.append(i//64)
            idxs = list(set(head_idxs))
