...
from ...dependency import Dependency, Node
import logging
import os
...
class MetaPruner:
    """
        ...
        Args:
            ...
            * ch_sparsity_dict (Dict[nn.Module, float]): the same as pruning_ratio_dict. Default: None.
            ...
            # Support UKMP
            * channel_per_step (int): For iterative pruning, re-calculate parameter importance and prune fixed number of channels per step.
            * trigger (Dict): For some special handling after pruning some modules.
            * save_imp (bool): Save parameter importance values for further analysis.
            * group_collect (str): How to aggregate importance across the parameters of an extire attention head.
            * multimodal (bool): The balance of parameter importance across modalities.
            * pre_imp (bool): For faster pruning process.
        """
    ...
    def __init__(
        ...
        self.channel_per_step = channel_per_step
        ...
        self.save_imp = save_imp
        self.group_collect = group_collect
        self.multimodal = multimodal
        self.pre_imp = pre_imp
        if pre_imp:
            self.pre_imp_dict = {}
    ...
    def step(self, interactive=False)-> typing.Union[typing.Generator, None]:
        self.current_step += 1
        if self.global_pruning:
            if self.channel_per_step != 0:
                if self.multimodal:
                    pruning_method = self.prune_global_step_multimodal
                else:
                    pruning_method = self.prune_global_step
            else:
    ...
    def estimate_importance(self, group, ch_groups=1) -> torch.Tensor:
        if isinstance(self.importance, list):
            return [imp(group, ch_groups) for imp in self.importance]
        else:
            return self.importance(group, ch_groups)
    ...
    def prune_local(self) -> typing.Generator:
        ...
        for group in self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types, root_instances=self.root_instances):
            if self._check_pruning_ratio(group): # check pruning ratio
                ...
                imp = self.estimate_importance(group, ch_groups)
            ...
            if len(global_head_importance)>0 and n_heads_removed>0:
                if group in global_head_importance:
                    ...
                    for qkv_layer in qkv_layers:
                        ...
                        self.out_channel_groups[qkv_layer] -= len(head_pruning_indices)
    ...
    def prune_global_step(self) -> typing.Generator:
        raise NotImplementedError("Please refer to maskpruner.py for iterative pruning of the UKMP project.")
        
    def is_visual_part(self, group):
        for dep, _ in group:
            module = dep.target.module
            if hasattr(module, 'weight') and ("visual" in module.weight.global_name or "vision" in module.weight.global_name):
                return True
        return False
        
    def prune_global_step_multimodal(self) -> typing.Generator:
        raise NotImplementedError("Please refer to maskpruner.py for multimodal iterative pruning of the UKMP project.")
        
    def update_head_memory(self, head_dim):
        for group in self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types, root_instances=self.root_instances):
            _is_attn, qkv_layers = self._is_attn_group(group)
            if _is_attn and self.prune_num_heads:
                for qkv_layer in qkv_layers:
                    self.num_heads[qkv_layer] = qkv_layer.out_features // head_dim # update num heads after pruning
                    self.out_channel_groups[qkv_layer] = qkv_layer.out_features // head_dim
                    
    def update_parameter_memory(self):
        for group in self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types, root_instances=self.root_instances):
            for dep, _ in group:
                if dep.target.type == ops.OPTYPE.PARAMETER: 
                    # update unwrapped nn.Parameter
                    old_parameter = dep.target.module
                    name = self.DG._param_to_name[old_parameter]
                    self.DG._param_to_name.pop(old_parameter)
                    path = name.split('.')
                    module = self.DG.model
                    for p in path[:-1]:
                        module = getattr(module, p)
                    pruned_parameter = getattr(module, path[-1])
                    self.DG._param_to_name[pruned_parameter] = name
                    self.DG.module2node[pruned_parameter] = self.DG.module2node.pop(old_parameter)
                    self.DG.module2node[pruned_parameter].module = pruned_parameter 