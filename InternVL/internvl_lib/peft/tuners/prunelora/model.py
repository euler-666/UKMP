...
from .layer import Conv2d, PruneLoraLayer, dispatch_default
...
class PruneLoraModel(LoraModel):
    """
    ...
    Example:
        ...
        >>> from .. import LoraModel, LoraConfig
        ...
        >>> import torch
        >>> import transformers
        >>> from .. import LoraConfig, PeftModel, get_peft_model, prepare_model_for_kbit_training
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        pruned_features = lora_config.pruned_features.get(target_name_key, None)
        pruned_r = lora_config.pruned_r
        ...
        if isinstance(target, PruneLoraLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
    ...
    def delete_adapter(self, adapter_name: str) -> None:
        ...
        for key in key_list:
            _, target, _ = _get_submodules(self.model, key)
            if isinstance(target, PruneLoraLayer):
