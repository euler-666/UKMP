...
@dataclass
class PruneLoraConfig(PeftConfig):
    """
    This is the configuration class to store the configuration of a [`PruneLoraModel`].

    Args:
        pruned_features (`Optional[dict]`): 
            The dimension of pruned Linear layers, for initializing the weight recalling branch.
        pruned_r (`int`):
            Lora attention dimension (the "rank") of the weight recalling branch.
    ...
    pruned_features: Optional[dict] = field(default=dict, metadata={"help": "pruned out/in features, if empty, not use"})
    pruned_r: int = field(default=8, metadata={"help": "Lora attention dimension for pruned branch"})
    ...
    def __post_init__(self):
        self.peft_type = PeftType.PRUNELORA
        self.target_modules = (
