...
class Blip2T5(Blip2Base):
    ...
    def forward_with_vision_auxloss(self, samples):
        ...
        # vision_auxloss =  - torch.abs(image_embeds).mean()
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            vision_auxloss = self.t5_model.direct_forward_classifier_loss(x=inputs_embeds)

            return {"loss": loss, "vision_auxloss": vision_auxloss}

    def get_logits_without_labels(self, samples):
        image = samples["image"]
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            decoder_tokens = torch.LongTensor(
                [self.t5_model.config.decoder_start_token_id]
            ).unsqueeze(0).expand(inputs_t5.shape[0], 1).to(image.device)
